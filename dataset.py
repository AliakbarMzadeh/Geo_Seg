...
class VesselSegmentMeshDataSet(dataset):
    ...
    def __getitem__(self, index):
        # Read raw image with SimpleITK to get metadata
        image_sitk = sitk.ReadImage(self.images[index])
        origin = image_sitk.GetOrigin()
        spacing = image_sitk.GetSpacing()
        size = image_sitk.GetSize()

        # Read mesh CSV and normalize to voxel space
        verts = np.asarray(list(map(convert, get_csv(self.meshes[index], delimiter=' '))))
        verts = (verts - origin) / spacing
        verts = 2. * verts / size[0] - 1.
        verts = verts[np.random.choice(len(verts), 3000)]

        # Load actual voxel array from image and label
...
def get_dataset():
    dire = '/content/converted_dataset'
    json_path = '/content/converted_dataset/data.json'
