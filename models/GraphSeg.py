...
class GraphSeg(nn.Module):
    ...
    def forward(self, img_feats):
        print('🧠 Entered GraphSeg.forward')
        ...
        print('🌀 Init verts and edges')
        x1_proj = torch.cat([
            self.gcn_projn[idx](images=feats, verts=init_verts)
            for idx, feats in enumerate(img_feats)
        ], dim=2)
        print('🧱 GCN Block #1 — feature projection')
        x1, x_hidden = self.gcn_model[0](x=torch.cat([x1_proj, init_verts], dim=2), edge_index=init_edges[0])
        print('🔁 GCN Block #1 — graph conv')
        x1 = x1 + init_verts
        x1_unpool = self.gcn_unpol(x=x1, edge_index=init_edges[0])
        x1_hidden_unpool = self.gcn_unpol(x=x_hidden, edge_index=init_edges[0])
        x2_proj = torch.cat([
            self.gcn_projn[idx](images=feats, verts=x1_unpool)
            for idx, feats in enumerate(img_feats)
        ], dim=2)
        print('🧱 GCN Block #2 — feature projection')
        x2, x_hidden = self.gcn_model[1](x=torch.cat([x1_hidden_unpool, x2_proj, x1_unpool], dim=2), edge_index=init_edges[1])
        print('🔁 GCN Block #2 — graph conv')
        x2 = x2 + x1_unpool
        x2_unpool = self.gcn_unpol(x=x2, edge_index=init_edges[1])
        x2_hidden_unpool = self.gcn_unpol(x=x_hidden, edge_index=init_edges[1])
        x3_proj = torch.cat([
            self.gcn_projn[idx](images=feats, verts=x2_unpool)
            for idx, feats in enumerate(img_feats)
        ], dim=2)
        print('🧱 GCN Block #3 — feature projection')
        x3, _ = self.gcn_model[2](x=torch.cat([x2_hidden_unpool, x3_proj, x2_unpool], dim=2), edge_index=init_edges[2])
        print('🔁 GCN Block #3 — graph conv')
        x3 = self.gcn_final(x3, init_edges[2])
        print('🏁 Final GCN Layer')
        x3 = x3 + x2_unpool
        print('✅ Finished GraphSeg.forward')
        return [x1, x2, x3], init_faces


def create_model(**kwargs):
    return GraphSeg(**kwargs)
