...
class Trainer(object):
    ...
    def load_parames(self, checkpoint, module, key):
        module.load_state_dict(torch.load(checkpoint, weights_only=False)['networker'][key])
    ...
    def init_network(self):
        ...
        for key, item in modules_param.items():
            module = source_import(item['defin_path']).create_model(**item['defin_parm'])
    ...
    def trainer(self):
        ...
        for index, data in tqdm(enumerate(self.train_dataloader), total=len(self.train_dataloader)):
            print(f'🧪 Iteration {index} — starting forward pass...')
            loss, pred, true = self.batrain(index, data)
            print(f'✅ Iteration {index} — loss computed: {loss}')
            train_loss += loss
    ...
    def metrics(self, y_pred, y_true, isTrain=False):
        ...
        label = "TRAIN" if isTrain else "EVAL"
        print(f"[{label}][Epoch {self.current_epoch}] Dice: {np.mean(dices):.4f} | Chamfer: {np.mean(chamf):.4e}")
